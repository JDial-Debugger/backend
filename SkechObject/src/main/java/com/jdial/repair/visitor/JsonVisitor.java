...
public class JsonVisitor extends jsonBaseVisitor<JsonNode> {
	...
	public Root visitJson(jsonParser.JsonContext ctx) {
		return new Root((Code) visit(ctx.code()), (Trace) visit(ctx.traces()), (Stdin) visit(ctx.stdin()));
	}
	...
	@Override
	public Trace visitTraces(jsonParser.TracesContext ctx) {
		List<TracePoint> t = new ArrayList<TracePoint>();
		for(int i = 0; i < ctx.trace().size(); i ++){
			TracePoint newTrace = (TracePoint) visit(ctx.trace(i));
			if(newTrace != null) {
				t.add((TracePoint) visit(ctx.trace(i)));
			}
		}
		return new Trace(t);
	}
	...
	@Override
	public TracePoint visitTrace(jsonParser.TraceContext ctx) {
		Assertions assertions = null;
		if (ctx.assertions() != null) {
			assertions = (Assertions) visit(ctx.assertions());
		}
		TracePoint ret = new TracePoint(
