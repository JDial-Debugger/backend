...
public class SketchScript {
	...
	public SketchScript(
		...
		List<Statement> coeffDecls
			= this.coeffs.stream()
				.map(Coefficient::getDeclFunc)
				.collect(
					ArrayList<Statement>::new,
	...
	/**
	 * Generates a script to input into Sketch based on the current examples and
	 * code in this script
	 * 
	...
	private void addSyntacticCompares(List<Coefficient> coeffs) {
		...
		if (this.synDistanceCompares == null) {
			...
			this.synDistanceCompares.addStmt(
				new StmtVarDecl(TypePrimitive.int32type, SYN_DISTANCE, new ExprConstInt(0))
			);
		}
	...
	/**
	 * 
	 * Between all examples, finds the example with the longest trace and returns
	 * the size
	 * 
	...
	/**
	 * Generates statements to initialize global arrays for recording program state
	 * and saves a set of variable names that will be recorded
	 */
	private void setStateDecls() {
		...
		for (String finalName : this.getFinalNames()) {
			this.stateDecls.addStmt(
				new StmtVarDecl(TypePrimitive.int32type, finalName, new ExprConstInt(0))
			);
		}

		// creates an array to hold state for every variable in every relevant function
		for (Function func : this.relevantFuncs.values()) {
			for (String varName : func.getBody().getActiveVarNames(this.correctVarTypes)) {
				...
				Type varStateType
					= new TypeArray(
						new TypeArray(TypePrimitive.int32type, new ExprConstInt(arrayLengths)),
		...
		this.stateDecls.addStmt(
			new StmtVarDecl(TypePrimitive.int32type, FUNC_INVOKE_COUNT, new ExprConstInt(-1))
		);
	}
	...
	/**
	 * Sketch requires a 'constraint' function that specifies any constraints that
	 * must be followed when filling in the sketch holes. This function creates that
	 * function and places constraints for each example and minimizes the semantic
	...
	private Function getConstraintFunc() {
		...
		for (CorrectionExample example : this.examples) {
			List<Expression> paramVals
				= example.getProgramTrace()
					.getParamVals(this.targetFunc)
					.stream()
					.map(val -> new ExprConstInt(val))
					.collect(Collectors.toList());
			body.add(new StmtExpr(new ExprFuncCall(this.targetFunc, paramVals)));
			body.add(example.getSketchCorrectionAssertions(this.targetFunc));
		}
	...
	/**
	 * Constructs an array for each active variable to hold its values in a trace
	 * for each example
	 * 
	 * @param varName     - the name of the variable
	 * @param funcName    - the name of the function containing the variable
	 * @param traceLength - the size of the array for each example, if the current
	 *                    example has a length smaller than traceLength, it will pad
	 *                    the array with 0s
	 */
	private void addOriginalState(String varName, String funcName, int traceLength) {
		...
		for (int i = 0; i < this.examples.size(); ++i) {
			...
			for (int j = 0; j < traceLength; ++j) {
				...
				if (j >= exampleTracePts.size()) {
					...
					if (
						cur.getLocals().containsKey(varName) && cur.getFuncName().equals(funcName)
					) {
		...
		Type varStateType
			= new TypeArray(
				new TypeArray(TypePrimitive.int32type, new ExprConstInt(this.getMaxTraceSize())),
				new ExprConstInt(this.examples.size())
			);
		this.originalStateDecls.addStmt(
			new StmtVarDecl(
				varStateType,
	...
	/**
	 * Add statements to compare a given varable's values in the sketch script with
	 * the original values in the trace
	 * 
	 * @param varName    - the name of the variable to compute distance for
	 * @param funcName   - the name of the function containing the variable (deals
	 *                   with case of same varirable name in multiple functions)
	 * @param origLength - the length of the original state array
	 */
	private void addSemanticCompares(String varName, String funcName, int origLength) {

		if (this.semDistanceCompares == null) {
			this.semDistanceCompares = new StmtBlock();
			this.semDistanceCompares.addStmt(
				new StmtVarDecl(TypePrimitive.int32type, SEM_DISTANCE, new ExprConstInt(0))
			);
		}
		...
		// semantic distance += funcNameVarnameState[i][j] !=
		// originalFuncNameVarName[i][j]
		Statement compare
			= new StmtAssign(
				...
				new ExprBinary2(
					new ExprArrayRange(
						...
						Arrays.asList(
							new ExprArrayRange.RangeLen(new ExprVar(exampleCount)),
	...
	/**
	 * Recursively adds control flow information to each statement in the body of
	 * the function
	 * 
	 * @param func
	 */
